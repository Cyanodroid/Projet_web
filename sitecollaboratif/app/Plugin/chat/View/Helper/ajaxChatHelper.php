...
class AjaxChatHelper extends Helper {
	...
	*/
	function generate($key) {
		...
		$html[] = $this->Form->create('Chat', array('id' => $key . 'ChatForm', 'url' => array('controller' => 'chat', 'action' => 'post')));
		$html[] = $this->Form->input('key', array('type' => 'hidden', 'value' => $key));
  
	    $html[] = $this->Form->input('name', array('id' => $key . 'ChatName'));
	    $html[] = $this->Form->input('message', array('id' => $key . 'ChatMessage', 'type' => 'textarea'));
